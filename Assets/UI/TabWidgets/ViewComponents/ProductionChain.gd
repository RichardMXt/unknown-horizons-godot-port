...
func set_production_line(production_line: ProductionLineComponent, slot_storage: SlotStorageComponent):
  ...
  self.output.resource_type = produces_keys.get(0) if len(produces_keys) > 0 else ResourceConfig.Resources.NONE
  self.output.resource_amount = production_line.produces.get(produces_keys.get(0)) if len(produces_keys) > 0 else ResourceConfig.Resources.NONE
  update_progress_bar()
  update_resource_amount()
